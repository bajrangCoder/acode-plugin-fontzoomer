...
- Zoom in and out to change font size using the `Shift + Plus` and `Shift + Minus` keyboard shortcuts
- Zoom in and out to change font size using `Volume up & down button`
- Customizable keyboard shortcuts to fit your preferences
- Lightweight(only 12KiB) and easy to use

## How to use:

1) Install the FontZoomer plugin in Acode from `Settings > Plugins`.
2) To zoom in, press `Shift + Plus`. To zoom out, press `Shift + Minus`.
